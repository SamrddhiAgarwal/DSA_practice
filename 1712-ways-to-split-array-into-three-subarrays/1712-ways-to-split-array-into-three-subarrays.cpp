...
//             // auto left_idx_it = lower_bound(start_it, end_it, min_Pj);
//             if (left_idx <= i) left_idx = i + 1;
//             while (left_idx < n - 1 && P[left_idx] < min_Pj) {
//                 left_idx++;
//             }
//             // auto right_idx_it = upper_bound(start_it, end_it, max_Pj);
//             if (right_idx < left_idx) right_idx = left_idx;
//             while (right_idx < n - 1 &&  P[right_idx] <= max_Pj) {
//                 right_idx++;
//             }
//             // Calculate how many valid 'j' positions exist in this window
//             if (left_idx < right_idx) {
//                 int valid_options = right_idx-left_idx;
//                 count = (count + valid_options) % MOD;
//             }
//         }
...
public:
    int waysToSplit(vector<int>& nums) {
        ...
        int total_sum = nums[n - 1];
        int count = 0;
        int left_idx=1,right_idx=1;
        for (int i = 0; i < n - 2 && nums[i] <= total_sum / 3; ++i) {
            
            int min_Pj = 2 * nums[i];
            int max_Pj = (total_sum + nums[i]) / 2;
        
            if (left_idx <= i) left_idx = i + 1;
            while (left_idx < n - 1 && nums[left_idx] < min_Pj) {
                left_idx++;
            }
        
            if (right_idx < left_idx) right_idx = left_idx;
            while (right_idx < n - 1 &&  nums[right_idx] <= max_Pj) {
                right_idx++;
            }
